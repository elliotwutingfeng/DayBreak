...
SunCalc.getSolarTime = function (dateValue, lng, utcOffset) {
  ...
  const localSolarTime =
    date.getHours() + timeCorrection / 60 + date.getMinutes() / 60;
...
SunCalc.getMoonTimes = function (dateValue, lat, lng, inUTC) {
  ...
  for (let i = 1; i <= 26; i += 2) {
    const h1 =
      SunCalc.getMoonPosition(hoursLater(dateValue, i), lat, lng).altitude - hc;
    const h2 =
      SunCalc.getMoonPosition(hoursLater(dateValue, i + 1), lat, lng).altitude -
      hc;
  ...
  if (!rise && !set) {
    ...
    result.alwaysUp = false;
    result.alwaysDown = false;
    result.highest = new Date(
      hoursLater(dateValue, Math.min(rise, set) + Math.abs(set - rise) / 2)
    );
  } else {
    result.alwaysUp = false;
    result.alwaysDown = false;
...
SunCalc.moonTransit = function (rise, set, lat, lng) {
  ...
  if (rise) {
    tempTransitAfter = calcMoonTransit(
      riseValue,
      SunCalc.getMoonTimes(
        new Date(riseDate).setDate(day + 1),
        lat,
        lng
      ).set.valueOf()
    );
  ...
  if (set) {
    tempTransitBefore = calcMoonTransit(
      setValue,
      SunCalc.getMoonTimes(
        new Date(setDate).setDate(day - 1),
        lat,
        lng
      ).rise.valueOf()
    );
